...
class Visualizer extends SvgTracker {
  ...
  _addOperation({ name, operation, idx }) {
    ...
    const pos = this._getPos(created.ns, idx)
    const size = this._getSize(timeAlive.ns)
  ...
  _className(name) {
    return 'operation-rect ' + name.replace(/[.:\s]/g, '-').toLowerCase()
  }
